...
void opencl_runtime::initialize() {
    ...
    std::cout << "--------------------------------------" << std::endl;
    std::cout << "OpenCL 編譯成功！偵測到的 Kernel 列表: " << std::endl;
    std::cout << kernel_names << std::endl; // 它會印出 "dense_forward;dense_backward_wb;..."
    std::cout << "--------------------------------------" << std::endl;
}
...
//size = 1000 -> 4 -> 1
float reduction(cl::Buffer input, int size, int local_size) {
    
    auto& runtime = opencl_runtime::getInstance();
    static cl::make_kernel<cl::Buffer,cl::Buffer,cl::LocalSpaceArg,int> reduction_kernel(opencl_runtime::getInstance().get_program(), "reduction");
    int max_groups = (size + local_size - 1) / local_size;

    cl::Buffer buf_a(runtime.get_context(), CL_MEM_READ_WRITE, max_groups * sizeof(float));
    cl::Buffer buf_b(runtime.get_context(), CL_MEM_READ_WRITE, max_groups * sizeof(float));

    cl::Buffer* src = &input;
    cl::Buffer* dst = &buf_a;
    cl::Buffer* next_dst = &buf_b;
    ...
    while (current_n > 1) {
        ...
        reduction_kernel(
            cl::EnqueueArgs(runtime.get_queue(), cl::NDRange(num_groups * local_size), cl::NDRange(local_size)),
            *src, *dst, cl::Local(sizeof(float) * local_size), current_n
        );
    ...
    float result = 0;
    runtime.get_queue().enqueueReadBuffer(*src, CL_TRUE, 0, sizeof(float), &result);
    return result;
}